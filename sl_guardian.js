...
async function getOpenOrders(symbol) {
  ...
  const data = await bnSignedRequest('GET', '/fapi/v1/algo/futures/openOrders');
  const orders = data.orders || [];
  // Фильтруем по symbol (endpoint возвращает все)
...
async function cancelOrder(symbol, algoId) {
  ...
  return bnSignedRequest('DELETE', '/fapi/v1/algo/futures/order', { symbol, algoId });
}
...
async function placeStopMarket(position, stopPrice) {
  ...
  const params = {
    ...
    triggerPrice: priceStr,           // в algo API называется triggerPrice (не stopPrice)
  ...
  return bnSignedRequest('POST', '/fapi/v1/algo/futures/newOrder', params);
}
