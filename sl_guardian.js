...
async function getOpenOrders(symbol) {
  // Algo orders endpoint (после миграции 2025-12-09)
  const data = await bnSignedRequest('GET', '/fapi/v1/openAlgoOrders', { symbol });
  // Endpoint может вернуть массив напрямую или {orders: []}
  return Array.isArray(data) ? data : (data.orders || []);
}

// Найти существующий STOP_MARKET для этой позиции (reduceOnly=true или closePosition=true)
...
async function cancelOrder(symbol, algoId) {
  // Новый endpoint для отмены algo ордера
  return bnSignedRequest('DELETE', '/fapi/v1/algoOrder', { symbol, algoId });
}

// Поставить STOP_MARKET закрывающий всю позицию (через algo endpoint)
async function placeStopMarket(position, stopPrice) {
  ...
  const params = {
    ...
    type: 'STOP_MARKET',                // Binance algo API требует 'type', не 'orderType'
    triggerPrice: priceStr,
    closePosition: 'true',              // закрывает всю позицию (no quantity needed)
    workingType: 'MARK_PRICE',
    priceProtect: 'TRUE',               // в этом API uppercase TRUE/FALSE
  };
  ...
  return bnSignedRequest('POST', '/fapi/v1/algoOrder', params);
}
...
async function processPosition(position) {
  ...
  // СЬТЬЕСТВУЕТ стоп
  const currentSL = parseFloat(existingStop.triggerPrice);
  ...
  if (diffPct < -CONFIG.SL_ADJUST_THRESHOLD) {
    ...
    try {
      await cancelOrder(symbol, existingStop.algoId);
      await placeStopMarket(position, calc.targetSL);
      logAction('tightened', {
        symbol, side,
